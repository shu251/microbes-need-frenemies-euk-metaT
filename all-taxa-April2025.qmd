...
```{r}
head(kegg_tax_nonvent)
```


```{r}
# unique(kegg_tax_nonvent$KeggOrthology_A)
tmp <- kegg_tax_nonvent %>% 
...
tmp2 <- tmp %>% 
  filter(KeggOrthology_B == "Overview") %>% 
  filter(KeggOrthology_C == "Carbon metabolism")
tmp2
unique(tmp2$KeggOrthology_D)
```
...
long_outlier <- long_frenemies_annot_tax %>% 
  ...
  anomalize::anomalize(scaledTPM, method = c("iqr"), max_anoms = 0.2, alpha = 0.05)
table(long_outlier$anomaly)
36036/(439185+36036) # 7.5% outlier
```
...
### Pre-processing ahead of k-measn
...
kmeans_membership_all <- df_kmeans_cluster10_vent_pa_3 %>% 
  # select(cols = -c(.rownames, .cluster)) %>% 
  select(FXN_INFO = `.rownames`, CLUSTER = `.cluster`) %>% distinct()
  # separate(tax, into = c("SUPERGROUP_PHYLUM", "full_classification"), sep = "_") %>% 
  # select(SUPERGROUP_PHYLUM, KEGG_PFAM, CLUSTER) %>% distinct()

head(kmeans_membership_all)
table(kmeans_membership_all$CLUSTER)
# View(kmeans_membership_all %>% filter(CLUSTER == 1))
```
## Visualize k-means output

```{r}
frenemies_all_kmeans <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  # filter(VENT_BIN == "Vent") %>% 
  # filter(Domain == "Eukaryota") %>% 
  # filter(Supergroup != "Opisthokonta") %>%
...
```{r}
head(frenemies_all_kmeans)
unique(frenemies_all_kmeans$CLUSTER)
```


```{r, fig.height=6, fig.width=6}
frenemies_all_kmeans %>% 
  unite(SAMPLE_ID, VENT_FIELD, VENT_BIN, LOCATION, sep = "_") %>%
...
For each transcript assigned protein ID, they were placed into clusters based on presence/absence. Here, I can plot the breakdown of protein IDs found within each supergroup phylum and see what cluster they belong to. 

Envision a bar plot. X-axis is the number of protein ID transcripts, y-axis is supergrou phylum
