...
# colnames(wide_kmean_ventonly_clr_1)

# wide_kmean_ventonly_eulc_2 <- stats::dist(t(wide_kmean_ventonly_clr_1), method = "euclidean")
# dist_norm_metat <- stats::dist(t(normed_metat), method = "euclidean", upper = FALSE, diag = FALSE)
```
...
```{r}
?decostand
wide_kmean_ventonly_clr_1 <- decostand(wide_kmean_vent_only_0, method = "pa")
...
For each transcript assigned protein ID and taxonomic ID, they are placed into clusters. Here, I can plot the breakdown of protein IDs found within each supergroup phylum and see what cluster they belong to. 
...
heat_wcluster %>% 
  ...
  theme_classic() +
  theme(legend.title = element_blank(),
    axis.text.x = element_text(angle = 90, color = "black", hjust = 0, vjust = 0.5),
    axis.text.y = element_text(color = "black"),
    strip.text = element_text(size = 5),
    strip.background = element_blank()) +
  labs(x = "", y = "")
