...
long_frenemies_annot_tax <- long_frenemies_annot %>% 
  ...
  mutate(SUPERGROUP_PHYLUM = case_when(
    ...
    #
    Phylum == "Choanoflagellida" ~ "Opisthokonta-Choanoflagellata",
    #
  ...
  mutate(SUPERGROUP_PHYLUM_CLASS = case_when(
    SUPERGROUP_PHYLUM == "Alveolata-Ciliophora" ~ SUPERGROUP_PHYLUM_2,
    SUPERGROUP_PHYLUM == "Alveolata-Myzozoa" ~ SUPERGROUP_PHYLUM_2,
    SUPERGROUP_PHYLUM == "Alveolata-Dinophyta" ~ SUPERGROUP_PHYLUM_2,
    TRUE ~ SUPERGROUP_PHYLUM
  ))

# View(long_frenemies_annot_tax %>% 
       # filter(grepl("Hacrobia", SUPERGROUP_PHYLUM)))

# sort(unique(long_frenemies_annot_tax$SUPERGROUP_PHYLUM))
# sort(unique(long_frenemies_annot_tax$SUPERGROUP_PHYLUM_CLASS))
# sort(unique(long_frenemies_annot_tax$SUPERGROUP_PHYLUM_2))
# unique((filter(long_frenemies_annot_tax, Supergroup == "Alveolata"))$SUPERGROUP_PHYLUM)
```

```{r}
tax_output <- long_frenemies_annot_tax %>% 
  select(SEQ_ID, full_classification, SUPERGROUP_18S, SUPERGROUP_PHYLUM, SUPERGROUP_PHYLUM_CLASS) %>% 
  distinct()
# write.csv(tax_output, file = "tax-output-SEQID-08272025.csv")
```
...
key_taxa <- long_frenemies_annot_tax %>% 
  select(full_classification, Supergroup, Phylum, Class, Order, Family, 
         SUPERGROUP_0, SUPERGROUP_PHYLUM, SUPERGROUP_PHYLUM_CLASS) %>% 
  distinct()
```
...
long_frenemies_annot_tax <-  long_frenemies_annot_tax %>% 
  ...
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% # remove outlier
...
# ggsave("figures/heatmap-biogeochem_aug28.svg", width = 10, height = 18.5, device = "svg", limitsize = FALSE)
```
...
```{r}
head(long_frenemies_annot_tax)
unique(long_frenemies_annot_tax$LOCATION)
```
...
# ggsave("figures/euclidean-dist-wbars_aug28.svg", width = 14, height = 14, device = "svg", limitsize = FALSE)
```
# Taxonomic overview
...
wide_frenemies_taxa <- long_frenemies_annot_tax %>%
  ...
  filter(!(grepl("Unannotated",SUPERGROUP_PHYLUM_CLASS))) %>% 
  filter(scaledTPM>0) %>% 
  unite(TAXA, SUPERGROUP_0, SUPERGROUP_PHYLUM_CLASS, sep = ";") %>% 
  unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>%
...
heat_base <- data.frame(clr_wide_frenemies_taxa) %>% 
  ...
  separate(TAXA, into = c("GROUP", "SUPERGROUP_PHYLUM_CLASS"), sep = ";", remove = FALSE) %>% 
  ...
  ggplot(aes(x = SAMPLE_ORDER, y = SUPERGROUP_PHYLUM_CLASS, fill = CLR)) +
  geom_tile(color = "white", lwd = 0.25) +
  scale_fill_fermenter(type = "div", palette = 5) +
  # coord_equal() +
  facet_grid(rows = vars(GROUP), space = "free", scale = "free") +
  scale_x_discrete(position = "top") +
  theme_classic() +
  theme(legend.title = element_blank(),
        legend.position = "left",
    ...
    strip.text.y = element_text(angle = 0, color = "black", hjust = 0, vjust = 0),
    strip.background = element_blank()) +
  labs(x = "", y = "")

heat_base
# ggsave("figures/heatmap-taxa_aug28.svg", width = 7, height = 13, device = "svg", limitsize = FALSE)
```




### Heatmap Alveolata - vent only

Repeat, but only include ciliates and dinoflagellates.

#### Ciliates
```{r}
wide_frenemies_ciliate <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
  ...
  mutate(CLASS = case_when(
           is.na(Class) ~ "Ciliophora-Other",
           Class == "SG_Alveolata_Indeterminate" ~ "Ciliophora-Other",
           TRUE ~ Class)) %>% 
...
clr_wide_frenemies_ciliate <- compositions::clr(wide_frenemies_ciliate)

frenemies_ciliate_rich <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
...
Order taxonomic names and sample IDs.
```{r}
tax_order_alpha <- sort(rownames(data.frame(clr_wide_frenemies_ciliate)), decreasing = TRUE)
sample_order_alpha <- sort(colnames(data.frame(clr_wide_frenemies_ciliate)))
# ?sort()
label_sample_order <- c("Candelabra GR", "Mt. Edwards GR", "SirVentsAlot GR", "Venti Latte GR", "LotsOShrimp P", "Shrimpocalypse P", "Arrow Loop VD", "Bartizan VD", "Old Man Tree VD", "Ravelin #2 VD", "Shrimp Hole VD", "White Castle VD", "X18 VD")

# head(data.frame(clr_wide_frenemies_ciliate))
```


```{r, fig.height=8, fig.width=8}
heat_base <- data.frame(clr_wide_frenemies_ciliate) %>% 
  rownames_to_column(var = "TAXA") %>% 
  pivot_longer(cols = -c(TAXA), names_to = "SAMPLE_ID", values_to = "CLR") %>% 
  separate(SAMPLE_ID, into = c("VENT_BIN", "VENT_FIELD", "LOCATION"), sep = "_", remove = FALSE) %>% 
  mutate(SAMPLE_ORDER = factor(SAMPLE_ID, levels = sample_order_alpha, labels = label_sample_order),
...
# head(frenemies_ciliate_rich)
order_tax <- as.character(frenemies_ciliate_rich$CLASS)

(heat_base) + (frenemies_ciliate_rich %>% 
  mutate(TAX_ORDER = factor(CLASS, levels = rev(order_tax))) %>% 
  ggplot(aes(x = 1, y = TAX_ORDER, size = NUM)) +
  ...
  theme(legend.title = element_blank(),
    ...
    strip.text = element_blank(),
    strip.background = element_blank()) +
  labs(x = "", y = "")) + patchwork::plot_layout(nrow = 1)
...
```{r}
# unique(long_frenemies_annot_tax$Phylum)
tmp <- long_frenemies_annot_tax %>% 
  filter(Phylum == "Dinophyta")
unique(tmp$Class)
head(long_frenemies_annot_tax)
```
...
```{r}
wide_frenemies_dino <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
  ...
  unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>%
  mutate(CLASS = case_when(
           is.na(Class) ~ "Dinophyta-Other",
           Class == "SG_Alveolata_Indeterminate" ~ "Dinophyta-Other",
           TRUE ~ paste(Class, Order, sep = "-"))) %>% 
  group_by(SAMPLE_ID, CLASS) %>%
    summarise(SUM_TPM = sum(scaledTPM)) %>% 
  pivot_wider(names_from = SAMPLE_ID, values_from = SUM_TPM, values_fill = 0) %>% 
  column_to_rownames(var = "CLASS") %>% 
...
clr_wide_frenemies_dino <- compositions::clr(wide_frenemies_dino)

frenemies_dino_rich <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
...
Order taxonomic names and sample IDs.
```{r}
tax_order_alpha <- sort(rownames(data.frame(clr_wide_frenemies_dino)), decreasing = TRUE)
sample_order_alpha <- sort(colnames(data.frame(clr_wide_frenemies_dino)))
# ?sort()
label_sample_order <- c("Candelabra GR", "Mt. Edwards GR", "SirVentsAlot GR", "Venti Latte GR", "LotsOShrimp P", "Shrimpocalypse P", "Arrow Loop VD", "Bartizan VD", "Old Man Tree VD", "Ravelin #2 VD", "Shrimp Hole VD", "White Castle VD", "X18 VD")
...
```{r, fig.height=8, fig.width=8}
heat_base <- data.frame(clr_wide_frenemies_dino) %>% 
  rownames_to_column(var = "TAXA") %>% 
  pivot_longer(cols = -c(TAXA), names_to = "SAMPLE_ID", values_to = "CLR") %>% 
  separate(SAMPLE_ID, into = c("VENT_BIN", "VENT_FIELD", "LOCATION"), sep = "_", remove = FALSE) %>% 
  mutate(SAMPLE_ORDER = factor(SAMPLE_ID, levels = sample_order_alpha, labels = label_sample_order),
...
```{r, fig.width=12, fig.height=8}
# head(frenemies_ciliate_rich)
order_tax <- as.character(frenemies_dino_rich$CLASS)

(heat_base) + (frenemies_dino_rich %>% 
  mutate(TAX_ORDER = factor(CLASS, levels = rev(order_tax))) %>% 
  ggplot(aes(x = 1, y = TAX_ORDER, size = NUM)) +
  geom_point(shape = 21, stroke = 1.1) +
  scale_x_discrete(position = "top") +
...
Try a comparison of overall vent to non-vent
```{r}
carb_lipid_df <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
  filter(SUPERGROUP_PHYLUM != "Eukaryote-Unannotated") %>% 
  filter(SUPERGROUP_PHYLUM != "Eukaryota incertae sedis-Unannotated") %>% 
  select(SEQ_ID, VENT_BIN, SUPERGROUP_PHYLUM, Supergroup, Phylum, Class, Order, SUPERGROUP_PHYLUM_CLASS, VENT_FIELD_ORDER, scaledTPM) %>% distinct() %>% 
  left_join(kegg_seq_id, by = c("SEQ_ID")) %>% 
  filter(B == "Carbohydrate and lipid metabolism")

head(carb_lipid_df)
# View(carb_lipid_df %>% select(C, D) %>% distinct())
# unique(carb_lipid_df$)
table(carb_lipid_df$C)
# unique(carb_lipid_df$SUPERGROUP_PHYLUM)

c_colors <- c("#2d88c9", "#8e9ba6", "#f6cc5e","#0f3e6d", "#ffded6",
              "#5bb648","#d4b387", "#c8acbd","#368989","#dd8798")
```

* Terpenoids are a series of C backbone structures that contribute to larger sterols or carotenoids. All terpenoids derive from a 5-carbon building c block, either "isopentenyl diphosphate" (IDP) or "dimethylallyl diphoshate" (DMADP). Acetyl-CoA is a key substrate that helps create these in a cell, and an isomerase works to further transform it. Common pathway - mevalonate (MEP pathway)

```{r, fig.width=30, fig.height=7}
library(ggtree)

carb_lipid_df %>% 
  # Select items of interest, remove duplication
  select(VENT_BIN, Supergroup, SUPERGROUP_PHYLUM, C, KEGG, scaledTPM) %>% distinct() %>% 
  filter(grepl("Alveolata", SUPERGROUP_PHYLUM) |
         grepl("Excavata", SUPERGROUP_PHYLUM) |
         grepl("Stramenopiles", SUPERGROUP_PHYLUM) |
         grepl("Hacrobia", SUPERGROUP_PHYLUM) |
         grepl("Rhizaria", SUPERGROUP_PHYLUM)) %>% 
  filter(!(grepl("-Unannotated", SUPERGROUP_PHYLUM))) %>% 
  group_by(VENT_BIN, SUPERGROUP_PHYLUM, Supergroup, C) %>% 
    summarize(gene_count = n(),
              sum_tpm = sum(scaledTPM)) %>% 
  ggplot(aes(area = sum_tpm, fill = C, size = 3)) +
  geom_treemap(color = "white") +
  facet_wrap(Supergroup + SUPERGROUP_PHYLUM ~ VENT_BIN, dir = "v", nrow = 2) +
  theme_linedraw() +
...
```
> Observations: Major non-venet to vent differences include, central carbohydrate metabolism, "other" carbohydrate metabolism, and fatty acid. 
* Many trends show increased central carbohydrate metabolism for the vent signature and fatty acid metabolism increase for non0vent. 
* One take home remains that there are clearly distinct strategies these taxa take within the vent sites compared to outside. 
...
carb_lipid_df %>% 
  ...
  group_by(VENT_BIN, SUPERGROUP_PHYLUM, Supergroup, C) %>% 
    summarize(gene_count = n(),
              sum_tpm = sum(scaledTPM)) %>% 
  ggplot(aes(area = sum_tpm, fill = C)) +
  geom_treemap(color = "white", size = 3) +
  facet_wrap(Supergroup + SUPERGROUP_PHYLUM ~ VENT_BIN, dir = "v", nrow = 4) +
  theme_linedraw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, face = "bold", size = 12),
        strip.background = element_blank(), strip.text = element_text(color = "black", face = "bold", vjust = 0.5, hjust = 0, size = 12),
        legend.position = "right",
...
```{r}
# View(carb_lipid_df %>% filter(C == "Central carbohydrate metabolism"))
```
...
Isolate ciliates, syndiniales, cercozoa
```{r, fig.width=18, fig.height=9}

subset <- c("Alveolata-Ciliophora", "Alveolata-Syndiniales", "Rhizaria-Cercozoa")

carb_lipid_df %>% 
  # Select items of interest, remove duplication
  select(VENT_BIN, Supergroup, SUPERGROUP_PHYLUM, Phylum, Class, Order, C, D, Gene_identification, KEGG, scaledTPM) %>% distinct() %>% 
  filter(grepl("Alveolata", SUPERGROUP_PHYLUM) |
         grepl("Excavata", SUPERGROUP_PHYLUM) |
         grepl("Stramenopiles", SUPERGROUP_PHYLUM) |
         grepl("Hacrobia", SUPERGROUP_PHYLUM) |
         grepl("Rhizaria", SUPERGROUP_PHYLUM)) %>% 
  filter(!(grepl("-Unannotated", SUPERGROUP_PHYLUM))) %>% 
  # filter(SUPERGROUP_PHYLUM %in% subset) %>% 
  group_by(VENT_BIN, SUPERGROUP_PHYLUM, C, D, Gene_identification) %>% 
    summarize(gene_count = n(),
              sum_tpm = sum(scaledTPM)) %>% 
  filter(C == "Central carbohydrate metabolism") %>% 
  # filter(!(is.na(Class))) %>% 
  ggplot(aes(x = Gene_identification, y = SUPERGROUP_PHYLUM, fill = log(sum_tpm))) +
  geom_tile(color = "black") +
  facet_grid(cols = vars(D), rows = vars(VENT_BIN),
             scales = "free", space = "free", switch = "both") +
  scale_fill_fermenter(type = "seq", palette = 5, direction = 1) +
  scale_x_discrete(position = "top") +
  scale_y_discrete(position = "right") +
  theme_classic() +
  theme(legend.title = element_blank(),
    ...
    strip.text = element_text(angle = 90, color = "black", hjust = 1, vjust = 0.5),
    strip.background = element_blank()) +
  labs(x = "", y = "")
```
...
```{r}
head(carb_lipid_df)
# View(carb_lipid_df %>% filter(C == "Central carbohydrate metabolism") %>% select(D) %>% distinct())
```

```{r}
close_up <- c("	
Citrate cycle, first carbon oxidation, oxaloacetate => 2-oxoglutarate", "Citrate cycle, second carbon oxidation, 2-oxoglutarate => oxaloacetate",
)
```
```{r}
head(carb_lipid_df)
...
carb_lipid_df %>% 
  # Select items of interest, remove duplication
  select(VENT_BIN, SUPERGROUP_PHYLUM, SUPERGROUP_PHYLUM_CLASS, Class,C, D, Gene_identification, KEGG, scaledTPM) %>% distinct() %>% 
  filter(grepl("Alveolata", SUPERGROUP_PHYLUM) |
         grepl("Excavata", SUPERGROUP_PHYLUM) |
         grepl("Stramenopiles", SUPERGROUP_PHYLUM) |
         grepl("Hacrobia", SUPERGROUP_PHYLUM) |
  ...
  filter(!(grepl("-Unannotated", SUPERGROUP_PHYLUM))) %>% 
  filter(SUPERGROUP_PHYLUM %in% subset) %>% 
  group_by(VENT_BIN, SUPERGROUP_PHYLUM, Class, C, D, Gene_identification) %>% 
    summarize(gene_count = n(),
              sum_tpm = sum(scaledTPM)) %>% 
  filter(C == "Central carbohydrate metabolism") %>% 
  filter(D != "Pentose phosphate pathway, archaea, fructose 6P => ribose 5P" &
           D != "Citrate cycle (TCA cycle, Krebs cycle)" &
           !(grepl("Entner-Doudoroff", D)) &
  D != "Pentose phosphate pathway, non-oxidative phase, fructose 6P => ribose 5P" &
  D != "Pentose phosphate pathway, oxidative phase, glucose 6P => ribulose 5P" &
    D != "PRPP biosynthesis, ribose 5P => PRPP") %>% 
  # Alter gene ids
  separate(Gene_identification, into = c("gene_tmp", "excess"), sep = ";") %>% 
  mutate(GENE = str_remove_all(gene_tmp, "E(\\d)\\.(\\d)\\.(\\d)\\.(\\d), ")) %>% 
  filter(!is.na(Class)) %>% 
  filter(SUPERGROUP_PHYLUM == "Alveolata-Ciliophora") %>% 
  ggplot(aes(x = Class, y = GENE, fill = log(sum_tpm))) +
  geom_tile(color = "grey50") +
  facet_grid(rows = vars(D), cols = vars(VENT_BIN),
  ...
  theme(legend.title = element_blank(),
        ...
        # axis.line = element_blank(),
        axis.ticks = element_blank(),
    axis.text.x = element_text(angle = 90, size = 8, color = "black", hjust = 0, vjust = 0),
    axis.text.y = element_text(color = "black", size = 8),
    panel.background = element_rect(color = "grey20"),
    strip.text.x = element_text(angle = 0, color = "black", size = 8, hjust = 1, vjust = 0.5),
    strip.text.y = element_text(angle = 0, color = "black", size = 8, hjust = 1, vjust = 0.5),
    strip.background = element_blank()) +
  labs(x = "", y = "")
```

1. Compare first vs. second part of TCA cycle
2. Glycolysis upregular, higher availability of organic carbon, microbes are more focused on breaking down sugars and materials, likely this means microbes are in rapi growth, respiration, fermnetation. vs. gluconeogenesis --> it means microves are relying on Non-carbohydrate sources for glucose and other downstream intermedates, Therefore it could be carbon availability is lower, they are trying to conserve energy.. maybe they are understress. 
...
```{r}
unique(carb_lipid_df$CLASSIFICATION_BROAD)
```

```{r, fig.height=11, fig.width=10}
carb_lipid_df %>% 
  # Select items of interest, remove duplication
  select(VENT_BIN, SUPERGROUP_PHYLUM, SUPERGROUP_PHYLUM_CLASS, Class,C, D, Gene_identification, KEGG, scaledTPM, CLASSIFICATION_BROAD) %>% distinct() %>% 
  filter(grepl("Alveolata", SUPERGROUP_PHYLUM) |
         grepl("Excavata", SUPERGROUP_PHYLUM) |
         grepl("Stramenopiles", SUPERGROUP_PHYLUM) |
         grepl("Hacrobia", SUPERGROUP_PHYLUM) |
  ...
  filter(!(grepl("-Unannotated", SUPERGROUP_PHYLUM))) %>% 
  filter(SUPERGROUP_PHYLUM %in% subset) %>% 
  group_by(VENT_BIN, SUPERGROUP_PHYLUM, Class, C, D, Gene_identification, CLASSIFICATION_BROAD) %>% 
    summarize(gene_count = n(),
              sum_tpm = sum(scaledTPM)) %>% 
  filter(CLASSIFICATION_BROAD == "Phagotrophy") %>% 
  separate(Gene_identification, into = c("gene_tmp", "excess"), sep = ";") %>% 
  mutate(GENE = str_remove_all(gene_tmp, "E(\\d)\\.(\\d)\\.(\\d)\\.(\\d), ")) %>% 
  filter(!is.na(Class)) %>% 
  # filter(SUPERGROUP_PHYLUM == "Alveolata-Ciliophora") %>% 
...
kob <- as.character(unique(tmp$KeggOrthology_B))
names(cluster_color) <- kob
# head(kegg_tax_vent$SUPERGROUP_PHYLUM)

tax_order_alpha <- as.character(sort(unique(kegg_tax_vent$SUPERGROUP_PHYLUM)))
...
kegg_tax_vent %>% 
  ...
  mutate(TAX_ORDER = factor(SUPERGROUP_PHYLUM, levels = rev(tax_order_alpha)),
...
> The above can make up a preliminary supplementary figure. 

# Kegg Orthology exploration

> Within each taxonomic group, what metabolisms are dominant among deep-sea vent protists?
...
```{r}
# unique(kegg_tax_nonvent$KeggOrthology_A)
tmp <- kegg_tax_vent %>% 
  filter(KeggOrthology_A == "Genetic Information Processing")
unique(tmp$KeggOrthology_B)

tmp2 <- kegg_tax_vent %>% 
  filter(B == "Carbohydrate and lipid metabolism") %>%
  select(B, Gene_identification) %>% distinct()
  # filter(KeggOrthology_C == "Carbon metabolism")
head(tmp2)
unique(tmp2$C)
length(unique(tmp2$Gene_identification))
```

### Carbon metabolism

Data frame `kegg_tax_vent` has been filtered already.
```{r}
head(kegg_tax_vent)
# unique(kegg_tax_vent$SUPERGROUP_PHYLUM)
```


```{r, fig.height=15, fig.width=8}
num_only_c <- kegg_tax_vent %>% 
  filter(B == "Carbohydrate and lipid metabolism") %>% 
  # filter(KeggOrthology_C == "Carbon metabolism") %>% #Try to subset C?
  unite(Carb_fxn, C, Gene_identification, sep = " ; ") %>% 
  select(Carb_fxn, SUPERGROUP_PHYLUM, scaledTPM) %>% distinct() %>% 
  filter(scaledTPM > 5) %>% 
  pivot_wider(names_from = SUPERGROUP_PHYLUM, values_from = scaledTPM, values_fill = 0, values_fn = mean) %>% 
  column_to_rownames(var = "Carb_fxn")

num_only_c_stand <- vegan::decostand(num_only_c, method = "range", MARGIN = 1)

data.frame(num_only_c_stand) %>% 
  rownames_to_column(var = "Carb_fxn") %>% 
  pivot_longer(cols = -c(Carb_fxn), names_to = "SUPERGROUP_PHYLUM", values_to = "RANK_VALUE") %>% 
  separate(Carb_fxn, into = c("C", "Gene_identification"), sep = " ; ") %>% 
  ggplot(aes(x = SUPERGROUP_PHYLUM, y = Gene_identification, fill = RANK_VALUE)) +
  geom_tile() +
  scale_fill_fermenter(type = "seq", palette = "Oranges", direction = 1) +
  facet_grid(rows = vars(C), scale = "free", space = "free") +
  scale_x_discrete(position = "top") +
  theme(axis.text.y = element_blank(),
        axis.text.x = element_text(angle = 90, color = "black", vjust = 1, hjust = 0),
        strip.text.y = element_text(angle = 0, color = "black", vjust = 1, hjust = 0))
```
...
metabolisms_of_interest_cellprocess <- c("Cell growth and death", "Cell motility", "Transport and catabolism")

cell_process <- kegg_tax_vent %>% 
  filter(KeggOrthology_B %in% metabolisms_of_interest_cellprocess) %>% 
  select(SUPERGROUP_PHYLUM, KeggOrthology_B, KeggOrthology_C, Gene_identification, scaledTPM) %>% 
  distinct() %>% 
  filter(grepl("Excavata", SUPERGROUP_PHYLUM) | 
           grepl("Amoebozoa", SUPERGROUP_PHYLUM) |
           grepl("Breviatea", SUPERGROUP_PHYLUM)) %>% 
  filter(!(grepl("Unannotated", SUPERGROUP_PHYLUM)))

## Modify by normalizing sclaedTPM with gene ID only.
tmp_cell_process <- cell_process %>% 
  group_by(SUPERGROUP_PHYLUM, Gene_identification) %>% 
  summarise(mean_TPM = mean(scaledTPM)) %>% 
  pivot_wider(names_from = SUPERGROUP_PHYLUM, values_from = mean_TPM, values_fill = 0) %>% 
  column_to_rownames(var = "Gene_identification")

tmp_cell_process_norm <- vegan::decostand(tmp_cell_process, na.rm = TRUE, method = "normalize") %>% 
  as.data.frame %>% 
  rownames_to_column(var = "Gene_identification") %>% 
  pivot_longer(cols = -c(Gene_identification), names_to = "SUPERGROUP_PHYLUM", values_to = "TPM_NORM")

head(tmp_cell_process_norm)
# unique(cell_process$KeggOrthology_C)
```

```{r, fig.width=18, fig.height=4}
rm <- c("Bacterial chemotaxis", "Meiosis - yeast", "Oocyte meiosis", "Cell cycle - Caulobacter", "Cell cycle - yeast", "Regulation of mitophagy - yeast")

cell_process %>% left_join(tmp_cell_process_norm) %>% 
  filter(!(KeggOrthology_C %in% rm)) %>% 
  separate(Gene_identification, into = c("GENE", "GENE_IDENTIFICATION"), sep = "; ") %>% 
  ggplot(aes(x = GENE, y = SUPERGROUP_PHYLUM, fill = TPM_NORM)) +
  geom_tile(stat = "identity") +
  facet_grid(cols = vars(KeggOrthology_B, KeggOrthology_C), scale = "free", space = "free") +
  ...
  theme(legend.title = element_blank(),
        ...
        # axis.line = element_blank(),
        axis.ticks = element_blank(),
    axis.text.x = element_text(angle = 90, color = "black", hjust = 1, vjust = 1),
    axis.text.y = element_text(color = "black"),
    # strip.text = element_blank(),
    strip.background = element_blank()) +
  labs(x = "", y = "")
...
colnames(kegg_tax_vent)
unique(kegg_tax_vent$KeggOrthology_B)
...
1. Subset samples as necessary.
2. Find and replace outliers.
3. Sum resulting transcripts by functional definition (PFAM + KEGG ID). Include distribution across. 

K-means clusters determined by TPM across samples. Two pre-treatment methods include converting to binary (p/a) and Euclidean distance.

## All data

Use all data, removing only Mustard Stand, determine outliers.

Find outliers. *Try with all data* Then try with >1 filter
...
long_outlier <- long_frenemies_annot_tax %>% 
  ...
  anomalize::anomalize(scaledTPM, method = c("iqr"), max_anoms = 0.2, alpha = 0.05)
table(long_outlier$anomaly)
36036/(439185+36036) # 7.5% transcripts considered outliers
```

Replace outliers with min and max values identified as non-outliers. 
...
wide_kmean_vent_only_0 <- long_frenemies_annot_tax %>% 
  ...
  pivot_wider(names_from = SAMPLE_ID, values_from = mean_TPM, values_fill = 0) %>% 
  column_to_rownames(var = "FXN_INFO") %>% 
  as.matrix
dim(wide_kmean_vent_only_0)
```
...
```{r}
# ?decostand
wide_kmean_ventonly_pa <- decostand(wide_kmean_vent_only_0, method = "pa", MARGIN = 2)
# 
```
...
kclusts <-
  tibble(k = 3:20) %>%
  mutate(
    kclust = map(k, ~ kmeans(wide_kmean_ventonly_pa, .x)),
...
output_kclusts %>%
  ...
  theme_linedraw()
```
> All data with presence/absence determined to be 11 clusters.


```{r}
kmeans10_vent_pa <- kmeans(wide_kmean_ventonly_pa, centers = 13)

summary(kmeans10_vent_pa)
# print(kmeans10_vent_pa)
broom::tidy(kmeans10_vent_pa)
broom::glance(kmeans10_vent_pa)

# Combine with data
augment_cluster10_vent_pa_3 <- broom::augment(kmeans10_vent_pa, wide_kmean_ventonly_pa)

df_kmeans_cluster10_vent_pa_3 <- (data.frame(augment_cluster10_vent_pa_3))

head(df_kmeans_cluster10_vent_pa_3)
# colnames(df_kmeans_cluster10_all_clr_3)
```

*Output for all data, P/A, and 11 k-means clusters*: K-means clustering with 11 clusters of sizes 2748, 1814, 933, 1050, 4560, 1668, 1764, 2123, 1464, 1290, 3084


> Row names are functional definitions and column headers are sample ID.


```{r, fig.height=4, fig.width=10}
kmeans_membership_all_pa <- df_kmeans_cluster10_vent_pa_3 %>% 
  select(FXN_INFO = `.rownames`, CLUSTER = `.cluster`) %>% distinct()

head(kmeans_membership_all_pa)
```

> SARAH MAKE CLUSTER-BASED PLOTS

### All data: P/A Visualize k-means output

```{r}
frenemies_all_pa_kmeans <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  # filter(VENT_BIN == "Vent") %>% 
  # filter(Domain == "Eukaryota") %>% 
  # filter(Supergroup != "Opisthokonta") %>%
  ...
  # filter(SUPERGROUP_PHYLUM != "Eukaryota incertae sedis-Unannotated") %>% 
  # unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>%
  unite(FXN_INFO, KEGG, PFAMs, sep = "_") %>%
  left_join(kmeans_membership_all_pa)

```

```{r, fig.height=6, fig.width=6}
frenemies_all_pa_kmeans %>% 
  unite(SAMPLE_ID, VENT_FIELD, VENT_BIN, LOCATION, sep = "_") %>%
  filter(!is.na(CLUSTER)) %>% 
  group_by(SAMPLE_ID, CLUSTER) %>% 
    summarize(TRANSCRIPTS_IN_CLUSTER = n(),
              SUM_TRANSCRIPTS_IN_CLUSTER = sum(scaledTPM)) %>% 
  mutate(SAMPLE_NAME_ORDER = factor(SAMPLE_ID, levels = order_sample_names, labels = label_sample_names)) %>% 
  ggplot(aes(x = SAMPLE_NAME_ORDER, y = CLUSTER, size = TRANSCRIPTS_IN_CLUSTER)) +
...
frenemies_all_pa_kmeans %>% 
  unite(SAMPLE_ID, VENT_FIELD, VENT_BIN, LOCATION, sep = "_") %>%
  filter(!is.na(CLUSTER)) %>% 
  group_by(SUPERGROUP_PHYLUM_CLASS, CLUSTER) %>% 
    summarize(TRANSCRIPTS_IN_CLUSTER = n(),
              SUM_TRANSCRIPTS_IN_CLUSTER = sum(scaledTPM)) %>% 
  # mutate(SAMPLE_NAME_ORDER = factor(SAMPLE_ID, levels = order_sample_names, labels = label_sample_names)) %>% 
  ggplot(aes(x = CLUSTER, y = SUPERGROUP_PHYLUM_CLASS, size = TRANSCRIPTS_IN_CLUSTER)) +
    geom_point(stat = "identity", shape = 21)
```
> Observations: Not a whole lot of differences across clusters. Some absent by taxa and specific samples. 
...
With TPM > 1 filter and Hellinger transformation.

```{r}
# ?decostand()
# ?kmeans()
wide_kmean_ventonly_pa <- decostand(wide_kmean_vent_only_0, method = "hellinger", MARGIN = 1)
# hist(wide_kmean_ventonly_pa)
```
...
output_kclusts %>%
  ...
  theme_linedraw()
```
> All data with presence/absence determined to be 13 clusters.
...
*Output for all data, >1 TPM, Hellinfer, and 13 k-means clusters*: K-means clustering with 13 clusters of sizes 2470, 1093, 482, 585, 186, 1080, 3661, 225, 135, 2550, 1109, 1680, 2280
...
```{r, fig.height=4, fig.width=10}
kmeans_membership_all_hell <- df_kmeans_cluster10_vent_pa_3 %>% 
  select(FXN_INFO = `.rownames`, CLUSTER = `.cluster`) %>% distinct()
```

### All data: Hellinger Visualize k-means output

```{r}
frenemies_all_hell_kmeans <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  # filter(VENT_BIN == "Vent") %>% 
  # filter(Domain == "Eukaryota") %>% 
  # filter(Supergroup != "Opisthokonta") %>%
  ...
  # unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>%
  unite(FXN_INFO, KEGG, PFAMs, sep = "_") %>%
  left_join(kmeans_membership_all_hell)

```

```{r, fig.height=6, fig.width=6}
frenemies_all_hell_kmeans %>% 
  unite(SAMPLE_ID, VENT_FIELD, VENT_BIN, LOCATION, sep = "_") %>%
  filter(!is.na(CLUSTER)) %>% 
  group_by(SAMPLE_ID, CLUSTER) %>% 
    summarize(TRANSCRIPTS_IN_CLUSTER = n(),
...
## All samples, remove unannotated

Use all samples, remove Mustard Stand, unannotated, and determine outliers.
...
long_outlier <- long_frenemies_annot_tax %>% 
  ...
  anomalize::anomalize(scaledTPM, method = c("iqr"), max_anoms = 0.2, alpha = 0.05)
table(long_outlier$anomaly)
11174/(225128+11174) # 4.7% transcripts considered outliers
```
...
```{r}
kmeans10_vent_pa <- kmeans(wide_kmean_ventonly_pa, centers = 10)
...
```{r, fig.height=4, fig.width=10}
kmeans_membership_alleuk_pa <- df_kmeans_cluster10_vent_pa_3 %>% 
  select(FXN_INFO = `.rownames`, CLUSTER = `.cluster`) %>% distinct()
```

### All data, no unannotated P/A Visualize k-means output

```{r}
frenemies_alleuk_pa_kmeans <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  # filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>%
  # filter(Supergroup != "Opisthokonta") %>%
  ...
  # unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>%
  unite(FXN_INFO, KEGG, PFAMs, sep = "_") %>%
  left_join(kmeans_membership_alleuk_pa)

```

```{r, fig.height=6, fig.width=6}
frenemies_alleuk_pa_kmeans %>% 
  unite(SAMPLE_ID, VENT_FIELD, VENT_BIN, LOCATION, sep = "_") %>%
  filter(!is.na(CLUSTER)) %>% 
  group_by(SAMPLE_ID, CLUSTER) %>% 
    summarize(TRANSCRIPTS_IN_CLUSTER = n(),
...
### All data, no unannotated Hellinger 
...
```{r}
kmeans10_vent_pa <- kmeans(wide_kmean_ventonly_pa, centers = 9)
...
```{r, fig.height=4, fig.width=10}
kmeans_membership_alleuk_hell <- df_kmeans_cluster10_vent_pa_3 %>% 
  select(FXN_INFO = `.rownames`, CLUSTER = `.cluster`) %>% distinct()
```
...
```{r}
frenemies_alleuk_hell_kmeans <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  # filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>%
  # filter(Supergroup != "Opisthokonta") %>%
  ...
  # unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>%
  unite(FXN_INFO, KEGG, PFAMs, sep = "_") %>%
  left_join(kmeans_membership_alleuk_hell)

```

```{r, fig.height=6, fig.width=6}
frenemies_alleuk_hell_kmeans %>% 
  unite(SAMPLE_ID, VENT_FIELD, VENT_BIN, LOCATION, sep = "_") %>%
  filter(!is.na(CLUSTER)) %>% 
  group_by(SAMPLE_ID, CLUSTER) %>% 
    summarize(TRANSCRIPTS_IN_CLUSTER = n(),
...
# Incorporate kmeans output 

```{r}
head(kmeans_membership_all_pa)
```

All data, scaled TPM > 1 and outlier removed.
Add to dataframe it originated from:
```{r}
long_frenemes_kmeans <- long_frenemies_annot_tax %>% 
  unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>% 
  unite(FXN_INFO, KEGG, PFAMs, sep = "_", remove = FALSE) %>% # Does not include taxonomic information
  filter(FXN_INFO != "-_-") %>% # Remove unannotated
  separate_longer_delim(KEGG, delim = ",") %>%
  mutate(KEGG = str_remove_all(KEGG, "ko:")) %>% 
  left_join(kegg_key) %>% 
  left_join(kmeans_membership_all_pa) %>% 
  # Isolate just the kegg ID information
  select(FXN_INFO, KEGG, PFAMs, A, B, C, D, Gene_identification, KeggOrthology_A, KeggOrthology_B, KeggOrthology_C, CLUSTER, scaledTPM) %>% 
  distinct()
  
colnames(long_frenemes_kmeans)
head(long_frenemes_kmeans)
# table(long_frenemes_kmeans$CLUSTER)
```

```{r}
# View(long_frenemes_kmeans %>% filter(is.na(CLUSTER)))
```


KeggOrthology_B
```{r, fig.height=14, fig.width=7}
long_frenemes_kmeans %>% 
  filter(!is.na(CLUSTER)) %>% 
  filter(!is.na(KeggOrthology_A)) %>% 
  group_by(CLUSTER, KeggOrthology_A, KeggOrthology_B) %>% 
  summarise(SUM_TPM = sum(scaledTPM),
            COUNT_transcript = n()) %>% 
  ggplot(aes(x = COUNT_transcript, y = KeggOrthology_A)) +
    geom_bar(stat = "identity", position = "fill", color = "black", (aes(fill = KeggOrthology_B))) +
...
```{r}
long_frenemes_kmeans_tax <- long_frenemies_annot_tax %>% 
  unite(SAMPLE_ID, VENT_BIN, VENT_FIELD, LOCATION, sep = "_") %>% 
  unite(FXN_INFO, KEGG, PFAMs, sep = "_", remove = FALSE) %>% # Does not include taxonomic information
  filter(FXN_INFO != "-_-") %>% # Remove unannotated
  separate_longer_delim(KEGG, delim = ",") %>%
  ...
  left_join(kmeans_membership_all_pa) %>% 
  # Isolate just the kegg ID information
  select(FXN_INFO, SUPERGROUP_0, CLUSTER, scaledTPM) %>% 
  distinct()
  
colnames(long_frenemes_kmeans_tax)
head(long_frenemes_kmeans_tax)
table(long_frenemes_kmeans$CLUSTER)
```
...
KeggOrthology_B
```{r, fig.height=8, fig.width=8}
long_frenemes_kmeans_tax %>% 
  filter(!is.na(CLUSTER)) %>% 
  filter(!is.na(SUPERGROUP_0)) %>% 
  group_by(CLUSTER, SUPERGROUP_0) %>% 
  summarise(SUM_TPM = sum(scaledTPM),
            COUNT_transcript = n()) %>% 
  mutate(SUPERGROUP_0_ORDER = factor(SUPERGROUP_0, levels = SUPERGROUP_0_ordering)) %>%
  ggplot(aes(x = COUNT_transcript, y = CLUSTER, fill = SUPERGROUP_0_ORDER)) +
    geom_bar(stat = "identity", position = "fill", width = 0.8) +
  scale_fill_manual(values = tax_color_metaT) +
  # facet_grid(rows = vars(CLUSTER), space = "free", scale = "free") +
  theme_classic() +
...
Use vent only data, removing only Mustard Stand, determine outliers.
...
```{r}
kmeans10_vent_pa <- kmeans(wide_kmean_ventonly_pa, centers = 11)
...
### All data: Visualize k-means output
...
## ARCHIVE Heatmap addition plots

```{r}
heat_wcluster <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
  ...
  unite(FXN_INFO, KEGG, PFAMs, sep = "_") %>%
  filter(FXN_INFO != "-_-") %>% # Remove unannotated
  left_join(kmeans_membership_all) %>% 
  group_by(SUPERGROUP_PHYLUM, CLUSTER) %>%
    summarise(SUM_TPM = sum(scaledTPM))
...
clusters <- as.character(c(1:11))

kmeans_color <- c("#fcba03","#d8efa5","#f3820c","#d49255",
                   "#e212bd","#10c7f2","#dc8ef8", "#5c6bcc",
                   "#80ab32","#facd45","#59ba25","#186cdb",
                   "#9dd4be","#b3916b","#b0d49d","#14543b",
                   "#2e7d6c","#507400","#a5e6d8","#a5b6e6",
...
tax_order_means <- as.character(sort(unique(heat_wcluster$SUPERGROUP_PHYLUM)))

heat_wcluster %>% 
  mutate(TAX_ORDER = factor(SUPERGROUP_PHYLUM, levels = rev(tax_order_means))) %>%
  ggplot(aes( y = TAX_ORDER, x = SUM_TPM, fill = CLUSTER)) +
  geom_bar(stat = "identity", position = "fill", width = 0.8, color  = "white", lwd = 0.25) +
  scale_fill_manual(values = kmeans_color) +
  scale_y_discrete(position ="right") +
  theme_classic() +
  theme(legend.title = element_blank(),
    axis.text.x = element_text(angle = 0, color = "black", hjust = 0, vjust = 0.5),
    axis.text.y = element_text(color = "black"),
    strip.text = element_text(size = 5),
    strip.background = element_blank()) +
  labs(x = "", y = "")
...
## Heatmap addition - ciliates

```{r, fig.height=6, fig.width=6}
heat_wcluster_ciliates <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
...
clusters <- names(kmeans_color)

tax_order_means <- as.character(sort(unique(heat_wcluster_ciliates$CLASS)))

heat_wcluster_ciliates %>% 
  mutate(TAX_ORDER = factor(CLASS, levels = rev(tax_order_means))) %>%
  ggplot(aes( y = TAX_ORDER, x = SUM_TPM, fill = CLUSTER)) +
  geom_bar(stat = "identity", position = "fill", width = 0.8, color  = "white", lwd = 0.25) +
  geom_bar(stat = "identity", position = "fill", width = 0.8, color  = "white", lwd = 0.25) +
  scale_y_discrete(position ="right") +
...
## Heatmap addition - dinos

```{r, fig.height=6, fig.width=6}
heat_wcluster_dino <- long_frenemies_annot_tax %>%
  filter(sample_name != "Von Damm_Vent_MustardStand") %>% 
  filter(VENT_BIN == "Vent") %>% 
  filter(Domain == "Eukaryota") %>% 
  filter(Supergroup != "Opisthokonta") %>%
...
tax_order_means <- as.character(sort(unique(heat_wcluster_dino$CLASS)))

heat_wcluster_dino %>% 
  mutate(TAX_ORDER = factor(CLASS, levels = rev(tax_order_means))) %>%
  ggplot(aes( y = TAX_ORDER, size = (SUM_TPM), x = CLUSTER)) +
  geom_point(shape = 21) +
  scale_y_discrete(position ="right") +
  scale_fill_manual(values = kmeans_color) +
  scale_y_discrete(position ="right") +
...
Next steps are to...
* make all the observations abouve figures on slides 2, 3, and 4. Then look at membership of transcript IDs to the clusters... Look at major metabolism patterns from the KEGG assignment. 
* look at comments below figures on slide deck to determine some key patterns. 
